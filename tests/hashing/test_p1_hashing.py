...
from design_intelligence.contracts.core.hashing import canonical_json_bytes, compute_content_hash
...
def test_canonical_json_preserves_unicode_utf8() -> None:
    assert canonical_json_bytes({"name": "服装"}) == '{"name":"服装"}'.encode("utf-8")


def test_canonical_json_handles_nested_json_values() -> None:
    payload = {"z": [True, None, {"b": 2, "a": 1}], "a": "x"}
    assert canonical_json_bytes(payload) == b'{"a":"x","z":[true,null,{"a":1,"b":2}]}'
...
def test_canonical_json_rejects_non_string_object_keys() -> None:
    with pytest.raises(Exception):
        canonical_json_bytes({1: "not-json-object-key"})
